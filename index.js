...
function findLastEvenNumber(arr) {
  ...
  const foundLast = arr.findLast((found) => found % 2 === 0);
...
function getPositiveArrayValues(arr) {
  ...
  const iterArray = Array.from(iterator)
  // За допомогою методу filter() отримуємо масив лише з додатніми значеннями.
  const inArray = iterArray.filter((value) => value > 0)
...
function removeAndDouble(arr, startIdx) {
  ...
  const doubledArray = arr.map(double => double * 2)
...
function flattenArray(arr) {
  ...
  const straiArr = arr.flat(Infinity)
...
function expandArrayByN(arr, n) {
  ...
  const newArr = arr.flatMap((num) => Array(n).fill(num))
...
function findLongestWord(arr) {
  ...
  const maxLeght = arr.reduce((long, current) => {
...
function findDuplicateElements(arr) {
  ...
  const doubleElem = arr.filter((elem, index) => arr.indexOf(elem) !== index)
...
function capitalizeWords(arr) {
  ...
  const capitalWords = arr.map(word => word.charAt(0).toUpperCase() + word.slice(1));
  //Повертаємо слово
...
function calculateTotalPrice(arr) {
  ...
  const allPrice = arr.reduce((price, current) => price + current.price, 0)
