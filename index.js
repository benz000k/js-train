...
function customAssign(...objects) {
  ...
  const mergeObj = Object.assign({}, ...objects)
  // Перетворення числових значень на рядки, ітеруємо об'єкт за допомогою for in
  for(let value in mergeObj) {
  // якщо значення типу число конвертуємо його в рядок
  if(typeof mergeObj[value] === "number") {
    mergeObj[value] = String(mergeObj[value])
  }
}
...
function customEntries(obj) {
  ...
  const entArr = Object.entries(obj)
...
function customObjectFromEntries(entries) {
  ...
  const prossEnteris = entries.map(([key, value]) => {
  ...
  const newObj = Object.fromEntries(prossEnteris)
...
function filterObjectsByKey(arr, key) {
  ...
  const filters = arr.filter((obj) => obj.hasOwnProperty(key))
...
function getObjectValuesSum(obj) {
  ...
  const sum = values.reduce((accum, currentValue) => accum + currentValue, 0)
  // Повертаємо суму
  return sum
}
